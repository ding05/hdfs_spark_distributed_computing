...
# Copy the codes into the notebook.
...
def mapper(record):
    ...
    return [("Number of lines", 1)]  # Simply add 1 to the total number of lines.
...
# Put the files into the working dictionary ("/users/home/dni25").

# Create a TXT file with 8-line "Hello world".

# Run "cat test.txt | python mapper.py" in the Unix shell.

# Run "cat test.txt | python mapper.py | sort -k1,1 | python reducer.py" in the Unix shell.

# (b)

# If a word appears for only once, using this approach, we don't calcualte its occurrences. This decreases the computational expenses.